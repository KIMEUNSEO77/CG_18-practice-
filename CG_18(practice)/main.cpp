...
void main(int argc, char** argv)
{
	...
	glutCreateWindow("Tesk_18(practice)");
...
GLvoid DrawAxis(GLuint shaderProgramID)
{
	...
	GLuint colorLoc = glGetUniformLocation(shaderProgramID, "uColor");   // 색깔
	...
	glUniform3f(colorLoc, 0.0f, 0.0f, 0.0f); // 축은 검정색 등 원하는 색상
...
GLvoid DrawCube(GLuint shaderProgramID)
{
	...
	glUniform3f(colorLoc, 1.0f, 0.0f, 0.0f); // 빨간색
...
GLvoid DrawCone(GLuint shaderProgramID)
{
	...
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, &M[0][0]);
	glUniform3f(colorLoc, 1.0f, 0.5f, 0.0f);  // 주황색
